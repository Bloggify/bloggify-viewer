...
Viewer.on("blog_page", ctx => {
    ...
    sameTime([
        cb => Bloggify.getPageBySlug(Bloggify.adapter.options.routes.blog, cb)
      , cb => Bloggify.getArticles({
        ...
        if (pageInfo.hasNewer) {
            blogPage.newerArticles = `${PATH_ARTICLES}/page/${pageNumber - 1}`;
        }
        if (pageInfo.hasOlder) {
            blogPage.olderArticles = `${PATH_ARTICLES}/page/${pageNumber + 1}`;
        }
